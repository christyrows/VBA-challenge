...
Next ws
 ...
 LastSummaryRow = ws.Cells(Rows.Count, 11).End(xlUp).Row
        ...
        For k = 2 To LastSummaryRow
         If Cells(k + 1, 11).Value > Cells(k, 11).Value Then
            
            
            TickerMaxName = Cells(k, 9).Value
            
            TickerMaxValue = Cells(k, 11).Value
            
            ws.Range("P2").Value = TickerMaxName
            
            ws.Range("Q2").Value = TickerMaxValue
        ...
        If Cells(k + 1, 11).Value > Cells(k, 11).Value Then
            ...
            TickerMinName = Cells(k, 9).Value
            
            TickerMinValue = Cells(k, 11).Value
            
            ws.Range("P3").Value = TickerMinName
            
            ws.Range("Q3").Value = TickerMinValue
            Else
        ...
        If Cells(k + 1, 12).Value < Cells(k, 12).Value Then
            ...
            VolumeMaxName = Cells(k, 9).Value
            
            VolumeMaxValue = Cells(k, 12).Value
            
            ws.Range("P4").Value = VolumeMaxName
            
            ws.Range("Q4").Value = VolumeMaxValue
